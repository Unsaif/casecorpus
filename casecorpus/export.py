...
def export_all(settings: Settings, cat: Catalogue, fmt: str = "both") -> dict[str, int]:
    ...
    tables: dict[str, list[dict[str, Any]]] = {k: [] for k in ("documents", "individuals", "narratives", "phenotypes", "measurements", "enzyme_activities", "genetic_findings", "diagnoses", "treatments", "links")}
    ...
    for r in cat.iter_individuals():
        ...
        tables["individuals"].append({
            ...
            "narrative_chars": sum(u.get("chars", 0) for u in rec.get("case_narrative", [])), "narrative_units": len(rec.get("case_narrative", [])), "narrative_source": rec.get("narrative_source"),
            "quotes_total": (rec.get("_evidence_verification") or {}).get("total"), "quotes_in_anchor": (rec.get("_evidence_verification") or {}).get("in_anchor"),
            "quotes_in_document": (rec.get("_evidence_verification") or {}).get("in_document"),
        })
        for k, u in enumerate(rec.get("case_narrative", [])):
            tables["narratives"].append({"record_id": r["record_id"], "pmid": r["pmid"], "order": k, "anchor": u["anchor"], "chars": u.get("chars"), "source": u.get("source"), "text": u["text"]})
        for ph in rec.get("phenotypes", []):
            tables["phenotypes"].append({"record_id": r["record_id"], "pmid": r["pmid"], "text": ph.get("text"), "hpo_id": ph.get("hpo_id"), "hpo_label": ph.get("hpo_label"),
                                         ...
                                         "grounding": (ph.get("grounding") or {}).get("status"), "quote": (ph.get("evidence") or {}).get("quote"), "anchor": (ph.get("evidence") or {}).get("anchor"), "quote_in_anchor": ((ph.get("evidence") or {}).get("verification") or {}).get("in_anchor"), "quote_in_document": ((ph.get("evidence") or {}).get("verification") or {}).get("in_document")})
        for m in rec.get("measurements", []):
            ...
            tables["measurements"].append({"record_id": r["record_id"], "pmid": r["pmid"], "analyte_text": m.get("analyte_text"), "analyte_label": an.get("label"), "chebi": an.get("chebi"), "vmh": an.get("vmh"),
                                           ...
                                           "grounding": (m.get("grounding") or {}).get("status"), "quote": (m.get("evidence") or {}).get("quote"), "anchor": (m.get("evidence") or {}).get("anchor"), "quote_in_anchor": ((m.get("evidence") or {}).get("verification") or {}).get("in_anchor"), "quote_in_document": ((m.get("evidence") or {}).get("verification") or {}).get("in_document")})
        ...
        for g in rec.get("genetic_findings", []):
            tables["genetic_findings"].append({"record_id": r["record_id"], "pmid": r["pmid"], **{k: g.get(k) for k in ("gene_symbol", "hgnc_id", "variant_hgvs_c", "variant_hgvs_p", "variant_hgvs_g", "transcript", "variant_text", "zygosity", "inheritance_as_stated", "classification_as_reported", "method", "novel")},
                                               "grounding": (g.get("grounding") or {}).get("status"), "quote": (g.get("evidence") or {}).get("quote"), "anchor": (g.get("evidence") or {}).get("anchor"), "quote_in_anchor": ((g.get("evidence") or {}).get("verification") or {}).get("in_anchor"), "quote_in_document": ((g.get("evidence") or {}).get("verification") or {}).get("in_document")})
        for d in rec.get("diagnoses", []):
            ...
            tables["diagnoses"].append({"record_id": r["record_id"], "pmid": r["pmid"], "disease_text": d.get("disease_text"), "label": dis.get("label"), "mondo": dis.get("mondo"), "omim": dis.get("omim"), "orphanet": dis.get("orphanet"),
                                        ...
                                        "quote": (d.get("evidence") or {}).get("quote"), "anchor": (d.get("evidence") or {}).get("anchor"), "quote_in_anchor": ((d.get("evidence") or {}).get("verification") or {}).get("in_anchor"), "quote_in_document": ((d.get("evidence") or {}).get("verification") or {}).get("in_document")})
